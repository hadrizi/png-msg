...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let raw_chunks = &value[8..];
        let mut chunks: (Vec<Vec<u8>>, u32) = (Vec::new(), 0);
        raw_chunks.iter().fold(&mut chunks, fold_chunk_bytes);
        match chunks
            .0
            .iter()
            // filter empty container obtained from `fold_chunk_bytes`
            .filter(|c| !c.is_empty())
            .map(|c| Chunk::try_from(&c[..]))
            .collect()
        {
            Ok(chunks) => Ok(Self { chunks }),
            Err(e) => Err(e),
        }
    }
}
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        match r {
            ...
            None => bail!("chunk {} not found", chunk_type),
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let bytes = Vec::from(Self::STANDARD_HEADER);
        self.chunks.iter().fold(bytes, |mut v, c| {
            v.append(&mut c.as_bytes());
            v
        })
    }
}

fn fold_chunk_bytes<'a>(c: &'a mut (Vec<Vec<u8>>, u32), b: &u8) -> &'a mut (Vec<Vec<u8>>, u32) {
    if c.0.is_empty() {
        c.0.push(Vec::new());
    }

    // To split flat bytes array into PNG chunks we will utilize this alghorithm
    //      1. Check if length of target chunk
    //      2. If it's equal 4 then we calculate length of data segment and push next byte to the chunk
    //      3. If it's greater than 4 then we push next byte to the chunk nad decrement length
    //      4. If it's less than 4 then we just push next byte to the chunk
    let last = c.0.last_mut().unwrap();
    match &last.len() {
        4 => {
            c.1 = u32::from_be_bytes(
                last.iter()
                    .copied()
                    .collect::<Vec<u8>>()
                    .try_into()
                    .unwrap(),
            ) + 7; // note `+ 7` here, this can be broken up into `+ 8 - 1`, because we push byte right aftewards
            last.push(*b);
        }
        other if *other > 4 => {
            last.push(*b);
    ...
    // Note that this will cause our collection of chunks to end with empty container
    if last.len() > 4 && c.1 == 0 {
        c.0.push(vec![]);
    }
...
mod tests {
    ...
    fn test_valid_from_bytes() {
        let expected_chunks = &testing_chunks();
        let chunk_bytes: Vec<u8> = expected_chunks
            .into_iter()
        ...
        assert!(png.is_ok());
        assert_eq!(png.unwrap().chunks(), expected_chunks);
    }
